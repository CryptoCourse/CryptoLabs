...
код, коллизии для хэш-функций (список hex-значений, 100 значений для наибольшего размера выхода хэш-функции), графики (времени и оценок затрат памяти). 

**NB**. Дабы точки не "кучковались" около нуля полезно было бы построить эти графики в логарифмическом масштабе по одной, или обеим осям.
...
**NB.** Лаборатоную работу можно выполнить как с использованием многопоточного выполнения, так и с "симуляцией" потоков, когда вы итеративно сначала выполняете действия первого потока, а затем второго.
...
**NB 4.** Уважаемые любители python - используйте человеческое и общепринятое отображение hex строк, а не то, что вам выводит print при передаче ему байтового массива.
