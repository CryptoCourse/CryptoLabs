...
#### 2.3 Реализовать интерфейс `byte[] ComputeMac(byte[] key, byte[] data)` - вычисляющий код аутентичности для прозвольных данных,
используя метод `MacAddBlock`

#### 2.4 Реализовать интерфейс `byte[] VerifyMac(byte[] key, byte[] data, byte[] tag)` - проверяющий код аутентичности для прозвольных данных,
