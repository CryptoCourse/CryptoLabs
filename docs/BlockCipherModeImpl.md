...
Рассмотрим, как пример, режим CBC. Явно IV в данном методе использовать не будем. Вместо этого реализуем следующую логику в начале метода:
...
Для реализации режимов, нам нужна функция "одноблочного шифрования блочного шифра AES" (та самая "корочка" с лекции, которая n бит в n бит отобразит). Почти всегда библиотеки не дают доступа к такой функции, но дают доступ к режиму ECB, который на одном блоке даёт как раз нужную нам функцию. Ровно это и надо сделать в `BlockCipherEncrypt`.
...
**NB.** `ProcessBlockEncrypt/ProcessBlockDecrypt` являются открытым api доспутным пользователю. Результат итеративного вызова `ProcessBlockEncrypt/ProcessBlockDecrypt`
на блоках данных должен соответствовать результату вызова `Encrypt/Decrypt`.
