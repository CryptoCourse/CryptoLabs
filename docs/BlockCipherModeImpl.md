...
**NB.** Способ передачи значения `IV` обычно остаётся на усмотрение пользователя библиотеки. Иногда для этого используются специальные структуры данных (когда необходимо помимо `IV` передать некоторые метаданные о шифровании (например режим и выбранный шифр), в которые помещается `IV` в специальное для него поле. Если же необходимо передавать только шифртекст - `IV` часто просто конкатенируется в его начало:

`c = IV || E(k, m)`

При расшифровании получатель знает длину `IV` (размер блока) и может разбить `c` на `IV` и шифртекст.

**NB.** `IV` не должен быть секретным при передаче по каналу связи, единственные требования на него - случайность (равномерное распределение) и уникальность для каждого шифруемого сообщения. Отсюда - при каждом шифровании одного и того же открытого текста мы получим новый шифртекст, так как используется новый случайный `IV`.
