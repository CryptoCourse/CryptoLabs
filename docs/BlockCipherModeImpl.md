...
- при выставленном флаге `isFinalBlock` в конце `ProcessBlockEncrypt` нужно корректно подготовить значение счётчика (по rfc) для шифрования сл. сообщения в режиме CTR. Для режимов CBC,OFB,CFB - сгенерировать новый IV для шифрования следующего сообщения.
